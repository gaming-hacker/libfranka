...
auto limitRate(double max_velocity,
                          double max_acceleration,
                          double max_jerk,
                          const Eigen::Vector3d& commanded_velocity,
                          const Eigen::Vector3d& last_commanded_velocity,
                          const Eigen::Vector3d& last_commanded_acceleration) -> Eigen::Vector3d {
  // Differentiate to get jerk
  Eigen::Vector3d commanded_jerk =
      (((commanded_velocity - last_commanded_velocity) / kDeltaT) - last_commanded_acceleration) / kDeltaT;
  ...
  if (commanded_jerk.norm() > kNormEps) {
    commanded_acceleration += (commanded_jerk / commanded_jerk.norm()) *
                              std::max(std::min(commanded_jerk.norm(), max_jerk), -max_jerk) * kDeltaT;
  }
...
auto limitRate(const std::array<double, 7>& max_derivatives,
                                const std::array<double, 7>& commanded_values,
                                const std::array<double, 7>& last_commanded_values) -> std::array<double, 7>{
...
auto limitRate(double max_velocity,
                 ...
                 double last_commanded_acceleration) -> double {
...
auto limitRate(const std::array<double, 7>& max_velocity,
                                ...
                                const std::array<double, 7>& last_commanded_accelerations) -> std::array<double, 7> {
...
auto limitRate(
    ...
    const std::array<double, 6>& last_O_ddP_EE_c) -> std::array<double, 6> {
...
auto limitRate(
    ...
    const std::array<double, 6>& last_O_dP_EE_c,
    const std::array<double, 6>& last_O_ddP_EE_c) -> std::array<double, 16> {
  ...
  std::array<double, 6> commanded_O_dP_EE_c{};
