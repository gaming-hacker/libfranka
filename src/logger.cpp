...
auto Logger::flush() -> std::vector<Record>{
  ...
  for (size_t i = 0; i < ring_size_; i++) {
    ...
    command.joint_positions.q = commands_[wrapped_index].motion.q_c;
    command.joint_velocities.dq = commands_[wrapped_index].motion.dq_c;
