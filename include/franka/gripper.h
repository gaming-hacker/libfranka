...
class Gripper {
 public:
   ...
   */
   auto operator=(Gripper&& gripper) noexcept -> Gripper&;
   ...
   */
  [[nodiscard]] auto homing() const -> bool;
   ...
   * @throw NetworkException if the connection is lost, e.g. after a timeout.
   */
  [[nodiscard]] auto grasp(double width,
             double speed,
             double force,
             double epsilon_inner = 0.005,
             double epsilon_outer = 0.005) const -> bool;
   ...
   * @throw NetworkException if the connection is lost, e.g. after a timeout.
   */
  [[nodiscard]] auto move(double width, double speed) const -> bool;
   ...
   */
   [[nodiscard]] auto readOnce() const -> GripperState;
  ...
  auto operator=(const Gripper&) -> Gripper& = delete;
  /// @endcond
